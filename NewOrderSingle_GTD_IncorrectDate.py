...
print ("{0:^100}".format("NewOrderSingle_GTD_IncorrectDate") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        #date -u +%s%N -d "01 May 2017"        
        nos.populate("ExpireDate",1493596800000000000)
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            if (("NewOrderReject(blockLength=20, templateId=7002, schemaId=19781, version=1" in  str(message)) and ("OrdRejReason=4000)" in str(message))):
                print("\n" + "TEST 2 OK")
            else:
                print("\n" + "TEST 2 FAIL")
        c.check(on)
