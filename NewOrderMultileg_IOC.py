...
print ("{0:^100}".format("NewOrderMultileg_IOC") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            if (("NewOrderMultilegResponse(blockLength=65, templateId=7001, schemaId=19781, version=1, ClOrdID=1" in  str(message)) and ("ClOrdLinkID=1, Side=1)" in str(message))):
                print("\n" + "TEST 2 OK")
            elif ("OrderCancelResponse(blockLength=44, templateId=7003, schemaId=19781, version=1, ClOrdID=1" in  str(message)):
                print("\n" + "TEST 3 OK")
            else:
                print("\n" + "TEST FAIL")
        time.sleep(1)        
