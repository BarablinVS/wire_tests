...
print ("{0:^100}".format("NewOrderSingle_CheckLimit") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            if (("NewOrderSingleResponse(blockLength=65, templateId=7000, schemaId=19781, version=1, ClOrdID=1" in  str(message)) and ("ClOrdLinkID=1, Side=1)" in str(message))):
                print("\n" + "TEST 2 OK")
            else:
                print("\n" + "TEST 2 FAIL")
        c.check(on)
