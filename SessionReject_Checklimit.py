...
print ("{0:^100}".format("SessionReject_5_Checklimit") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            if ("SessionReject(blockLength=13, templateId=5008, schemaId=19781, version=1, ClOrdID=1, RefTagID=20217, SessionRejectReason=5)" == str(message)):
                print("\n" + "TEST 2 OK")
            else:
                print("\n" + "TEST 2 FAIL")
