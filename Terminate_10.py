...
print ("{0:^100}".format("Terminate_10") + "\n")
...
cmd = "cd /app/fusion/wire_gate/scripts;./stop.sh"
process = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
temp = process.communicate()[0]
      
def on(message):
    print str(datetime.datetime.now()) +" IN: "+ str(message)
    if ("Terminate(blockLength=1, templateId=5003, schemaId=19781, version=1, TerminationCode=10)" ==  str(message)):
        print("\n" + "TEST 2 OK")
    else:
        print("\n" + "TEST 2 FAIL")
...
cmd2 = "cd /app/fusion/wire_gate/scripts;./start.sh"
