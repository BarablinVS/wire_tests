...
print ("{0:^100}".format("OrderReplaceRequest_Mode_1") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            print(orderId)
            if (("OrderReplaceResponse(blockLength=60, templateId=7005, schemaId=19781, version=1, ClOrdID=241" in str(message)) and (("PrevOrderID=" + str(orderId)) in  str(message))
                and ("OrderQty=5" in  str(message)) and ("ClOrdLinkID=242" in str(message))):
                print("\n" + "TEST 3 OK")
            else:
                print("\n" + "TEST 3 FAIL")
        c.check(on)
