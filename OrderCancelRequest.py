...
print ("{0:^100}".format("OrderCancelRequest") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        ocr.populate("ClOrdID",239)
        ocr.populate("Account", account_2)
        ocr.populate("OrderID", orderId)
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            print(orderId)
            if (("OrderCancelResponse(blockLength=44, templateId=7003, schemaId=19781, version=1, ClOrdID=239" in str(message)) and (("OrderID=" + str(orderId)) in  str(message)) and ("ClOrdLinkID=1" in str(message))):
                print("\n" + "TEST 3 OK")
            else:
                print("\n" + "TEST 3 FAIL")
        c.check(on)
