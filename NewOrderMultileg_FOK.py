...
print ("{0:^100}".format("NewOrdermultileg_FOK") + "\n")
...
def send(i=1):
    ...
    while i < 2:
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            if (("NewOrderReject(blockLength=20, templateId=7002, schemaId=19781, version=1, ClOrdID=1" in  str(message)) and ("OrdRejReason=4103)" in str(message))):
                print("\n" + "TEST 2 OK")
            else:
                print("\n" + "TEST 2 FAIL")
        c.check(on)
