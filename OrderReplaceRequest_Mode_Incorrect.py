...
print ("{0:^100}".format("OrderReplaceRequest_Mode_Incorrect") + "\n")
...
#                print(orderId)
        ...
        def on(message):
            print str(datetime.datetime.now()) +" IN: "+ str(message)
            print(orderId)
            if ("SessionReject(blockLength=13, templateId=5008, schemaId=19781, version=1, ClOrdID=240, RefTagID=20019, SessionRejectReason=5)" == str(message)):
                print("\n" + "TEST 3 OK")
            else:
                print("\n" + "TEST 3 FAIL")
        c.check(on)
